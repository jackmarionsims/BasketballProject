...
import pandas as pd
from classes2 import NBAName, NBATeam, Date
...
DataFrame Modifiers
'''
def calculate_avgs_for_team(df, nba_name: NBAName, season) -> NBATeam:
...
def calculate_elos(df, season, all_teams):
...
def get_pregame_stats(home: NBATeam, visitor: NBATeam, date: str) -> pd.DataFrame:
    ...
    def avg(stats, col, games):
        return stats.get(f"Total {col}", 0) / games if games > 0 else 0

    # Build game ID
    date_str = date_to_id(home.name.value, date)
    team_abbr = NAME_TO_ABBR[home.name.value]
    game_id = f"{date_str}0{team_abbr}"
    ...
    home_w  = home_stats.get("Total W", 0)
    home_l  = home_stats.get("Total L", 0)
    vis_w   = visitor_stats.get("Total W", 0)
    vis_l   = visitor_stats.get("Total L", 0)
    ...
    row.update({
        ...
        "Home Team W":              home_w,
        "Home Team L":              home_l,
        "Home Team W/L%":           home_w / home_games if home_games > 0 else 0,
        "Visitor Team Total Games": visitor_games,
        "Visitor Team W":           vis_w,
        "Visitor Team L":           vis_l,
        "Visitor Team W/L%":        vis_w / visitor_games if visitor_games > 0 else 0,
    })
