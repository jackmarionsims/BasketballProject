...
predictors = ['PTS Margin', 'W/L% Margin', 'ELO Margin',
              'Tot EFF Margin', "Home Team PTS Avg", "Visitor Team PTS Avg"]
...
y_margin = df["Home Team PTS"] - df["Visitor Team PTS"]
...
for year in years:
    ...
    y_margin_train = all_train["Home Team PTS"] - all_train["Visitor Team PTS"]

    X_test = all_test[predictors]
    y_home_test = all_test["Home Team PTS"]
    y_visitor_test = all_test["Visitor Team PTS"]
    y_margin_test = all_test["Home Team PTS"] - all_test["Visitor Team PTS"]
    ...
    margin_preds_mean = margin_model.predict(X_test)
    ...
    margin_rounded = np.round(margin_preds_mean).astype(int)
    for i in range(len(home_rounded)):
        if home_rounded[i] == visitor_rounded[i]:
            if (home_preds_mean[i] >= visitor_preds_mean[i]):
                ...
                visitor_rounded[i] += 1
        # if margin_rounded[i] == 0:
        #     if (margin_preds_mean[i] >= 0):
        #         home_rounded[i] += 1
        #     else:
        #         home_rounded[i] -= 1
    ...
    preds["Predicted Margin"] = margin_rounded.copy()
...
margin_predicted_wins = [1 if row["Predicted Margin"] > 0 else 0 for _, row in all_predictions.iterrows()]
years_predicted = df[df["Season"] >= 1991]
print(accuracy_score(years_predicted["Home Win"], predicted_wins ))
print(accuracy_score(years_predicted["Home Win"], margin_predicted_wins ))
